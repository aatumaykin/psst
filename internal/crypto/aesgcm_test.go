...
func TestDeriveKeyFromPassword_RejectsWrongSaltSize(t *testing.T) {
	a := NewAESGCM()
	_, err := a.DeriveKeyFromPassword("mypassword", []byte("short"), DefaultKDFParams())
	if err == nil {
		t.Fatal("DeriveKeyFromPassword should reject undersized salt")
	}

	longSalt := make([]byte, saltSize+1)
	_, err = a.DeriveKeyFromPassword("mypassword", longSalt, DefaultKDFParams())
	if err == nil {
		t.Fatal("DeriveKeyFromPassword should reject oversized salt")
	}

	emptySalt := []byte{}
	_, err = a.DeriveKeyFromPassword("mypassword", emptySalt, DefaultKDFParams())
	if err == nil {
		t.Fatal("DeriveKeyFromPassword should reject empty salt")
	}
}
