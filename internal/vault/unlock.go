...
func (v *Vault) Unlock(ctx context.Context) error {
	...
	if timeStr, _ := v.store.GetMeta(ctx, "kdf_time"); timeStr != "" {
		return v.unlockGitVault(ctx, rawKey, timeStr)
	}
	...
	v.resetUnlockState(ctx)
	return nil
}

func (v *Vault) unlockGitVault(ctx context.Context, rawKey []byte, timeStr string) error {
	saltB64, err := v.store.GetMeta(ctx, "kdf_salt")
	if err != nil {
		return fmt.Errorf("get kdf_salt: %w", err)
	}
	if saltB64 == "" {
		return errors.New("vault corrupted: kdf_salt missing")
	}
	salt, decodeErr := base64.StdEncoding.DecodeString(saltB64)
	if decodeErr != nil {
		return fmt.Errorf("decode kdf_salt: %w", decodeErr)
	...
	aadStr, err := v.store.GetMeta(ctx, "vault_aad")
	if err != nil {
		crypto.ZeroBytes(key)
		return fmt.Errorf("get vault_aad: %w", err)
	}
...
func (v *Vault) resetUnlockState(ctx context.Context) {
...
func metaAtoi(ctx context.Context, s store.MetaStore, key string) int {
	val, _ := s.GetMeta(ctx, key)
