...
- Unlock state per session: the `*vault.Vault` + `expiresAt`. **Inactivity timeout 30m, sliding**: every request from the session EXCEPT `GET /api/session` and requests carrying `X-Psst-Auto: 1` (10) refreshes `expiresAt = now + timeout` (a polling countdown tab must not keep the key alive forever; the UI counts down locally from `unlockExpiresAt` and re-syncs only on real actions). A background sweeper (1 tick/min) closes expired unlocks (keys zeroed) and deletes expired sessions.
