...
| Output | Full in-memory render, then a single write; on any hard error (including chmod) the output file contains zero rendered bytes (a pre-existing output file stays untouched) |
...
3. `${NAME}` — braced shell syntax. Inner must satisfy `isName` and membership, else literal (non-strict) / unresolved-shell (strict). `${VAR:-default}`-style defaults are NOT supported and stay literal (§7). An unterminated `${` with no closing `}` is literal text (parallel to rule 1).
