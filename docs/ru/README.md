...
- Принятие ротации отказывает, если в клоне есть локальные коммиты, не отправленные
  ...
  ключи): отправьте их через `psst sync` или отбросьте через
  `psst sync --discard-local` (значения остаются в reflog).
...
- Запущенному `psst serve` перезапуск не нужен: следующая операция вернёт запрос на
  повторный unlock; после `psst sync --accept-rotation` на хосте сервера unlock
  выполняется новым паролем.
