...
Требует git-хранилище (`psst migrate storage --to git`). Токен генерируется при
запуске и печатается один раз; можно передать его через `--token` (виден в выводе
`ps` на многопользовательских машинах) или `PSST_SERVE_TOKEN`. Пароль vault
вводится в интерфейсе (unlock, таймаут неактивности 30 минут). Значения секретов
показываются только по нажатию кнопки reveal.

Удалённый доступ: `ssh -L 7788:127.0.0.1:7788 <host>`, затем открыть
`http://127.0.0.1:7788` локально.

### Рендер шаблонов

```bash
psst render --in deploy.env.tpl --out deploy.env           # вывод всегда 0600
psst render --in config.yaml.tpl --out config.yaml --tag prod
psst render --in app.ini.tpl --out app.ini --strict
```

- `{{KEY}}` — должен разрешиться; неразрешённые имена завершают команду с ошибкой
  (fail-closed: буквальный `{{KEY}}` никогда не попадёт в результат). Примечание:
  шаблоны, смешанные с другой системой шаблонизации `{{ }}` (Helm, Go-шаблоны),
  отрендерить нельзя — каждый блок `{{...}}` должен разрешиться.
- `$KEY` / `${KEY}` — заменяется, если секрет существует, иначе остаётся как есть.
- `--strict` — неразрешённые `$`-плейсхолдеры тоже завершают команду с ошибкой.
- Результаты рендера — секреты в открытом виде: добавьте их в `.gitignore` (по
  умолчанию игнорируются только `.env`/`.env.*`); `psst scan` ловит утечки в
  git-отслеживаемых файлах.

Рецепты, не зависящие от протокола (прокси не нужен):
...
```bash
echo "new-password" | psst rotate --stdin   # старый пароль — через PSST_PASSWORD или промпт
psst sync --accept-rotation                 # на каждой другой машине
```

- `psst rotate` (только git-хранилище) генерирует новую соль и одним коммитом
  перешифровывает все секреты вместе с обновлённым `psst.yaml`; машина, выполнившая
  ротацию, сразу обновляет свой pin.
- `psst rotate --kdf` тем же коммитом дополнительно усиливает KDF-параметры до
  значений по умолчанию — никогда не ослабляет: на хранилище уже с параметрами по
  умолчанию это no-op для параметров (соль всё равно меняется), а на хранилище сильнее
  умолчаний команда падает отказом (CLI-пути ослабить параметры нет намеренно).
- На каждой другой машине старый пароль перестаёт работать с ошибкой
  `vault salt changed; run 'psst sync --accept-rotation' (or re-clone)`. Принятие
  ротации сначала проверяет НОВЫЙ пароль и лишь затем сдвигает pin — неверный пароль
  оставляет клон в состоянии отказа (молча pin никогда не обновляется), попытку можно
  повторить.
- Принятие ротации отказывает, если в клоне есть локальные коммиты, не отправленные
  в remote (rebase коммитов со старым ключом поверх ротации навсегда смешал бы
  ключи): отбросьте их через `psst sync --discard-local` (значения остаются
  в reflog) и повторите — обычный `psst sync` не сможет прошить старые коммиты
  после приземления ротации.
- Если `psst rotate` прервался на середине, remote не затронут — upstream и есть
  состояние до ротации; рабочее дерево сбрасывается через `psst sync --discard-local`.
- Запущенному `psst serve` перезапуск не нужен: пока ротация не принята на хосте
  сервера, следующая операция падает с ошибкой, называющей
  `psst sync --accept-rotation`, и закрывает все unlock; после приёма возможен один
  запрос на повторный unlock, затем unlock выполняется новым паролем.
