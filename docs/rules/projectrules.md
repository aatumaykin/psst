...
internal/
  ...
  store/                — SQLite + Git persistence (SecretStore interfaces)
  ...
  server/               — Web UI server (HTTP API, sessions, embedded SPA)
  render/               — Template rendering ({{KEY}}/$KEY substitution, leaf)
...
make test               # Run all tests (PSST_NO_KEYCHAIN=1 go test -race ./... -v)
